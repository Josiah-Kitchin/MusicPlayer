...
SongQueue::SongQueue() : headSong(nullptr), tailSong(nullptr), controller(nullptr), 
		         currentSongInfo("Select a Playlist", "", "", "", -1){
    isPlaying = false; 
    isPaused = true; 
    isSkipped = false; 
    isRepeat = false; 
    isPrevious = false; 
    isShuffle = false; 
    endQueue = false; //flag for ending the method begin queue  
} 
...
void SongQueue::enqueueSongsFromPath(const string playlistPath) { 
    ...
    vector<string> files; //pushed into a vector to implement the shuffle feature 
...
void SongQueue::begin(const string songPath) {
    //Begins the song queue at the given songPath. If the songPath is empty, it
    //starts the queue at the beggning. 
	...
	//Just in case the song isnt found (it should always be found though )
	if (!currentSong) { 
    ...
    isPlaying = true; 
    while (currentSong && !endQueue) {   
	
	//let the controller handle communicating with the view for the song info
	currentSongInfo = currentSong->info; 
	if (controller) { 
	    controller->signalSongChange(); 
	}
    ...
    if (controller) { //reset the view 
	SongInfo emptySong("Select a Playlist", "", "", "", -1); 
	currentSongInfo = emptySong; 
	controller->signalSongChange(); 
    }
    isPlaying = false; 
    endQueue = false; 
