...
SongQueue::SongQueue() : headSong(nullptr), tailSong(nullptr) {
    isPlaying = false; 
    isPaused = true; 
    isSkipped = false; 
    isRepeat = false; 
...
void SongQueue::beginQueue(const string songPath) {
	...
	//end testing 
	updateSongInfoLabels(currentSong->info); 
	
	playAudioFile(currentSong->songPath); 
...
vector<string> SongQueue::getSongsInQueue() { 
    //Returns a vector of all songs in the queue
    shared_ptr<Song> currentSong = headSong; 
    vector<string> songs; 
    while (currentSong) {
